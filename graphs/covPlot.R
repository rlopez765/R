...
con <- ghdbConnect(drv)
#rs <- dbSendQuery(con, "select pos,read_f_a+read_r_a+read_overlap_a*2+read_f_c+read_r_c+read_overlap_c*2+read_f_g+read_r_g+read_overlap_g*2+read_f_t+read_r_t+read_overlap_t*2 from ghm_snp where run_sample_id='A0548301_1';")
rs <- dbSendQuery(con, "select to_number(replace(replace(s.chrom,'X','23'),'Y','24'),'999999999999'),s.pos,read_f_a+read_r_a+read_overlap_a*2+read_f_c+read_r_c+read_overlap_c*2+read_f_g+read_r_g+read_overlap_g*2+read_f_t+read_r_t+read_overlap_t*2 from ghm_snp s, (select chrom,pos from gh_annotation_ss4 where aa_change!='' group by chrom,pos) a where run_sample_id='A0548301_1' and s.chrom=a.chrom and a.pos=s.pos order by s.chrom,s.pos limit 1000000;")
...
#pdf("/media/warp2/pipeline/SSNV4_RENE/A0548301_1_200Bins.pdf")
png("/media/warp2/pipeline/SSNV4_RENE/A0548301_1.png", width=2000)
